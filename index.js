...
switch (command) {
  case 'create':
    createProduct().then(done => { return console.log('Starting product creation...') });
    break
  case 'update':
    updateProduct().then(done => { return console.log('Starting update...') });
    break
  case 'delete':
    deleteProduct();
    break
  default:
    return console.log('Please supply a valid command such as create, update or delete.');
}
...
async function createProduct() {
  ...
  for (const item of products) {
    ...
    Moltin.Products.Create({
      ...
      console.log(`product ${done.data.name} with the ID of ${done.data.id} has beeen created.`)
    })
  }
...
async function updateProduct() {
  ...
  for (const item of products) {
    await waitFor(2000);
    Moltin.Products.Filter({ eq: { sku: item.sku } }).All()
      .then(update => {
...
function deleteProduct() {
  return console.log('This will delete a product');
}
